 ...
 */
export const LAB_IDS = ['ballon', 'fiole', 'eprouvette', 'goutte'] as const
 ...
 */
export const LAB_SHAPES: BotShape[] = LAB_IDS.map((id) => {
  const forme = SHAPE_BY_ID.get(id)
  if (!forme) throw new Error(`lab: la forme "${id}" n'est pas au catalogue`)
  return forme
})

/** Le type large du catalogue accepte-t-il cet id ? Sert aux valeurs relues du stockage. */
export const estFormeLab = (id: string): id is LabShapeId =>
  (LAB_IDS as readonly string[]).includes(id)
 ...
 */
export const LAB_DEFAUT: LabShapeId = 'ballon'

/** Garde-fou pour une valeur relue du stockage : rend toujours un id utilisable. */
export const formeLabOuDefaut = (id: string | null | undefined): ShapeId =>
  id && estFormeLab(id) ? id : LAB_DEFAUT
