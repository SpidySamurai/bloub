 ...
 */
function resous(epreuves, ancre) {
    if (!epreuves.length)
        return { x: 0, y: 0 };
 ...
 */
function visageDe(def, pose, expr, ecart) {
    const v = def.baseFace && expr
...
const NUL = { x: 0, y: 0 };
function decalagePour(def, radii, expr, ancre, ecart) {
    const epreuves = [];
    ...
    return new Map(SHAPES.map((forme) => {
        ...
        for (const def of STATES) {
            ...
            for (const expr of expressions) {
                par.set(clef(def.id, expr?.id ?? null), decalagePour(def, forme.radii, expr, forme.ancreVisage ?? NUL, forme.ecartVisage ?? 1));
            }
        }
...
//# sourceMappingURL=eyefit.js.map