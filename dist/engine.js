...
const materialise = (radii) => radii ? radii.map((r, i) => ({ x: r * Math.cos(ANGLES_MOTEUR[i]), y: r * Math.sin(ANGLES_MOTEUR[i]) })) : null;
...
export class BotEngine {
     ...
     */
    setShape(radii, now = 0, sansVisage = false, pts = null, ecartVisage = 1, yeuxPoses = false) {
        if (radii === this.shape &&
    ...
    facteurVisage(now) {
        if (this.muet === this.muetPrev)
            return this.muet ? 0 : 1;
        const k = (now - this.shapeAt) / BotEngine.SHAPE_MORPH;
        if (k >= 1)
            return this.muet ? 0 : 1;
        const t = easings.easeOutQuint(clamp(k));
    ...
    posed(def, t, shape, expr, pts = null, ecart = 1) {
        let pose = def.pose(t);
        ...
        if (ecart !== 1)
            pose = { ...pose, split: pose.split * ecart };
        return pose;
    }
    /**
     ...
     */
    origine(now, shape, expr, pts = null) {
        if (this.departFige)
    ...
    sample(now) {
        ...
        if (visage < 1)
            pose = { ...pose, eyeAlpha: pose.eyeAlpha * visage };
        // --- vie au repos -----------------------------------------------------
         ...
         */
        const bodyRadius = (x, y) => this.poses ? 1 : radiusAtAngle(pose.sil.radii, Math.atan2(y, x) - pose.sil.rot);
        const eyes = [];
