...
export function blend(a, b, t, out) {
    ...
    if (a.pts || b.pts) {
        ...
        const pts = dst.pts ?? new Array(PROFILE_SAMPLES);
 ...
 */
export function starProfile(points, rOuter, rInner, rc, rotationDeg = 0) {
    const rot = (rotationDeg * Math.PI) / 180;
    const n = points * 2;
 ...
 */
export function resampleOutline(poly, samples = PROFILE_SAMPLES) {
...
export const rayonMax = (pts) => Math.max(...pts.map((p) => Math.hypot(p.x, p.y)));
